...
const { getEthereumUSDTBalance } = require("./utils/blockchain/ethereum");
const { getTronUSDTBalance } = require("./utils/blockchain/tron");
...
router.get("/api/usdt-balance", async (req, res) => {
  ...
  if (!address || !network) {
    return res.status(400).json({
      error: errorMessages.general.missingParams,
    });
  }

  if (!constant.SUPPORTED_NETWORKS.includes(network)) {
    return res.status(400).json({
      error: errorMessages.general.unsupportedNetwork,
    });
  }

  try {
    let balance;
    if (network === constant.NETWORK.ETHEREUM) {
      balance = await getEthereumUSDTBalance(address);
    } else if (network === constant.NETWORK.TRON) {
    ...
    if (
      error.message === errorMessages.ethereum.invalidUsdtAddress ||
      error.message === errorMessages.ethereum.invalidAddress ||
      error.message === errorMessages.tron.invalidAddress ||
      error.message === errorMessages.tron.tronAddressError
    ) {
